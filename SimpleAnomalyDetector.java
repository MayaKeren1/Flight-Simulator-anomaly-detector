...
public class SimpleAnomalyDetector implements TimeSeriesAnomalyDetector {
	...
	public void learnNormal(TimeSeries ts) {
		...
		for(int i = 0; i < ts.list.get(0).length; i++)
		{
			index = -1;
			float maxi = (float)corr;
			float [] x=ts.getColum(arrName[i]);
			for(int j = i+1; j < ts.list.get(0).length; j++)
			{
			...
			if(index != -1)
			{
				...
				Point[] points = new Point[ts.list.size()-1];
				float [] maxcolum=ts.getColum(arrName[index]);//
				for(int k =0; k < ts.list.size()-1;k++)
				{
	...
	public List<AnomalyReport> detect(TimeSeries ts) {
		...
		for(int i=0;i<correl.size();i++)
		{
			...
			for(int j=0; j < ts.list.size()-1; j++)
			{
